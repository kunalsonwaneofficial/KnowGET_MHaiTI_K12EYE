...
| **Status**     | ✅ Complete — CI green; merged to main (`20c4d24`). In-sandbox: `@knowget/resource` typecheck/lint/format/build clean, **52 tests** (20 files); `apps/api` typecheck clean + resource DI-graph spec (2 tests); RLS verified on live PostgreSQL. Full monorepo build / DB-integration CI-verified (TD-12). |
