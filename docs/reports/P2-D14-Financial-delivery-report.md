...
| **Status**     | 🟡 Awaiting CI — branch `feat/p2-d14-financial` pushed. In-sandbox: `@knowget/financial` typecheck/lint/format/build clean, **63 tests** (20 files); `apps/api` typecheck clean + financial DI-graph spec (2 tests); RLS verified on live PostgreSQL. Full monorepo build / DB-integration are CI-verified (TD-12). |
